...
Features:
- Dynamic Dependency Installer (subprocess + pip)
- Startup Connection & Status Check Utility (opt-in via --check flag)
...
# ── 0. AUTO-INSTALL AND IMPORT DEPENDENCIES ───────────────────────────────

def install_and_import(package_name, import_name=None):
    """Checks if a package is installed; if not, installs it automatically."""
...
geopy_mod    = install_and_import("geopy")
plyer_mod    = install_and_import("plyer")
colorama_mod = install_and_import("colorama")

from geopy.distance import geodesic
from geopy.geocoders import Nominatim
from plyer import gps, notification

colorama_mod.init(autoreset=True)
...
HEADERS       = {"User-Agent": "personal-randonaut-script/0.5 (Local testing)"}
CONFIG_FILE   = "randonaut_config.json"
LOG_FILE      = "randonaut_log.json"

# Speed test uses a known-size file so the Mbps calculation is actually real.
# Cloudflare's 1MB test file is reliable and fast to fetch.
SPEED_TEST_URL       = "https://speed.cloudflare.com/__down?bytes=1000000"
SPEED_TEST_BYTES     = 1_000_000  # 1 MB
...
# ═══════════════════════════════════════════════════════════════════════════
# PRE-FLIGHT STATUS CHECK  (run with --check flag, not on every launch)
# ═══════════════════════════════════════════════════════════════════════════

def run_preflight_check():
    p_head("=== Pre-flight Diagnostics ===")

    # 1. Real download speed test using a 1MB Cloudflare test file
    print(f"{C_WARN}  Testing download speed (1MB test file)...{C_RESET}")
    notification_title = "Randonaut Check"
    status_msg = ""
    try:
        start = time.time()
        resp  = requests.get(SPEED_TEST_URL, timeout=20, stream=True)
        # Consume the response body so we measure actual transfer, not just TTFB
        bytes_received = sum(len(chunk) for chunk in resp.iter_content(chunk_size=8192))
        duration = time.time() - start
        speed_mbps = (bytes_received * 8) / (duration * 1_000_000)
        p_ok(f"Download speed: {speed_mbps:.2f} Mbps ({bytes_received/1000:.0f}KB in {duration:.2f}s)")
        status_msg = f"Download: {speed_mbps:.2f} Mbps"
        notification_title = "Randonaut: All Systems Go"
    except Exception as e:
        p_warn(f"Speed test failed: {e}")
        status_msg = "Speed test failed"
        notification_title = "Randonaut: Network Issue"

    # 2. Validate Nominatim / geopy is reachable
    print(f"{C_WARN}  Validating Nominatim (geopy)...{C_RESET}")
    try:
        geolocator = Nominatim(user_agent="randonaut-preflight/0.5")
        location   = geolocator.reverse("38.8977, -77.0365")
        p_ok(f"Nominatim OK — test location: {location.address[:60]}...")
    except Exception as e:
        p_warn(f"Nominatim check failed: {e}")

    # 3. Verify Overpass is reachable
    print(f"{C_WARN}  Checking Overpass API...{C_RESET}")
    try:
        test_q = '[out:json][timeout:5]; node(1); out;'
        resp   = requests.post(OVERPASS_MIRRORS[0], data={"data": test_q},
                                headers=HEADERS, timeout=10)
        if resp.status_code == 200:
            p_ok("Overpass API reachable")
        else:
            p_warn(f"Overpass returned HTTP {resp.status_code}")
    except Exception as e:
        p_warn(f"Overpass check failed: {e}")

    # 4. Verify OSRM is reachable
    print(f"{C_WARN}  Checking OSRM routing...{C_RESET}")
    try:
        test_url = f"{OSRM_BASE_URL}/walking/-77.0369,38.9072;-77.1872,38.7893"
        resp     = requests.get(test_url, params={"overview": "false"}, headers=HEADERS, timeout=10)
        data     = resp.json()
        if data.get("code") == "Ok":
            p_ok(f"OSRM OK — test route: {data['routes'][0]['distance']:.0f}m")
        else:
            p_warn(f"OSRM returned code: {data.get('code')}")
    except Exception as e:
        p_warn(f"OSRM check failed: {e}")

    # 5. Desktop notification
    print(f"{C_WARN}  Sending desktop notification...{C_RESET}")
    try:
        notification.notify(
        ...
        p_ok("Notification sent")
    except Exception as e:
        p_warn(f"Notification failed (non-critical): {e}")

    p_head("Diagnostics complete. Launching...\n")
...
def log_result(origin_lat, origin_lon, result, settings):
    ...
    try:
        ...
        p_info(f"Logged to {LOG_FILE} ({len(log)} total runs)")
    except Exception as e:
...
def get_hardware_gps(timeout_s=5):
    if not PLYER_AVAILABLE:
        p_info("Hardware GPS not available on this platform.")
        return None
    global _gps_location
    _gps_location = None
    try:
    ...
    try:
        ...
        for el in resp.json().get("elements", []):
            ...
            if not name or name in seen:
                continue
            seen.add(name)
            kind   = (tags.get("amenity") or tags.get("leisure") or tags.get("natural")
                      or tags.get("tourism") or tags.get("highway") or tags.get("shop") or "place")
            dist_m = geodesic((lat, lon), (el.get("lat", lat), el.get("lon", lon))).meters
...
if __name__ == "__main__":
    # Run preflight diagnostics only when explicitly requested
    # Usage: python randogit.py --check
    ...
    if result:
        ...
        if pois:
            ...
            print(f"\n  {C_WARN}  No named POIs within 250m — sounds remote!{C_RESET}")
